...
def coef2curve(x_eval, grid, coef, k = 10, device="cuda"):
    '''
...
def curve2coef(x_eval, y_eval, grid, k, device="cuda"):
    ...
    coef = torch.linalg.lstsq(mat.to(device), y_eval.unsqueeze(dim=2).to(device),
                              driver='gels' if device == 'cuda' else 'gels').solution[:, :, 0] #gelsy
...
class KANLayer(nn.Module):
    ...
    def __init__(self, in_dim=3, out_dim=2, num=5, k=10, noise_scale=0.1, scale_base=1.0, scale_sp=1.0, base_fun=torch.nn.SiLU(), grid_eps=0.02, grid_range=[-1, 1], sp_trainable=True, sb_trainable=True, device='cuda'):
        ''''
        initialize a KANLayer
